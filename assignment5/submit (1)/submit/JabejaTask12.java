...
public class Jabeja {
  ...
  // New SA parameters
  private static final float T_MIN = 0.00001f;
  ...
  public void startJabeja() throws IOException {
    for (round = 0; round < config.getRounds(); round++) {
      // Reset temperature to 1.0 at round 400 for SA restart
      if (round == 400) {
        this.T = 1.0f;
      }
  ...
  private double acceptanceProbability(double oldCost, double newCost, float temperature) {
    // Accept worse solutions with probability exp((newCost - oldCost) / T), gets smaller as T decreases or newCost much worse.
  ...
  public Node findPartner(int nodeId, Integer[] nodes){
    ...
    for (Integer qid : nodes) {
      ...
      int q_color = nodeq.getColor();
      if (nodep.getColor() == q_color)
        continue;
      ...
      double newBenefit = Math.pow(d_pq, config.getAlpha()) + Math.pow(d_qp, config.getAlpha());
      
      if (round < 400) {
        // JaBeJa original approach for first 400 rounds
        if (newBenefit * this.T > old && newBenefit > highestBenefit) {
          highestBenefit = newBenefit;
          bestPartner = nodeq;
        }
      } else {
        // Simulated Annealing approach after round 400
        if (newBenefit > old) {
          ...
          double ap = acceptanceProbability(old, newBenefit, this.T);
